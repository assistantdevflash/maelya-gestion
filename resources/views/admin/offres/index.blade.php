                    ...
                    <td>
                        <div class="flex items-center gap-2">
                            ...
                            <form action="{{ route('admin.offres.destroy', $offre) }}" method="POST"
                                  id="delete-offre-{{ $offre->id }}">
                                @csrf @method('DELETE')
                            </form>
                            <button type="button"
                                    @click="confirmDelete('{{ $offre->id }}', '{{ addslashes($offre->nom) }}')"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-lg border border-red-200 text-red-600 bg-red-50 hover:bg-red-100 transition">
    ...
    <div x-show="deleteOpen" x-cloak class="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4"
        ...
        <div @click.outside="deleteOpen = false" class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-sm p-6">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    <h3 class="font-bold text-gray-900 dark:text-white">Supprimer cette offre ?</h3>
                    <p class="text-sm text-gray-500 mt-0.5" x-text="'\"' + deleteNom + '\"' + ' sera définitivement supprimée.'"></p>
                </div>
            </div>
            <div class="flex justify-end gap-3">
                <button @click="deleteOpen = false" class="px-4 py-2 text-sm font-medium rounded-lg border border-gray-200 text-gray-600 hover:bg-gray-50 transition">Annuler</button>
                <button @click="submitDelete()" class="px-4 py-2 text-sm font-bold rounded-lg bg-red-600 text-white hover:bg-red-700 transition">Supprimer</button>
            </div>
        </div>
    </div>
...
function offresManager() {
    return {
        ...
        confirmDelete(id, nom) {
            this.deleteFormId = 'delete-offre-' + id;
            this.deleteNom = nom;
