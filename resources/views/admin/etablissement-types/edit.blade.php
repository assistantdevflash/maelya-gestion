...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div>
        ...
        <p class="text-gray-500 dark:text-slate-400 mt-1">
            {{ $etablissementType->libelle }}
        </p>
    </div>

    {{-- Formulaire --}}
    <form method="POST" action="{{ route('admin.etablissement-types.update', $etablissementType) }}" class="card">
        @csrf
        @method('PUT')
        <div class="card-body space-y-6">
            ...
            <div>
                ...
                <input type="text" name="libelle" value="{{ old('libelle', $etablissementType->libelle) }}" required autofocus maxlength="200"
            ...
            <div>
                ...
                <input type="text" name="code" value="{{ old('code', $etablissementType->code) }}" required maxlength="100"
                       class="form-input @error('code') border-red-400 @enderror"
                       placeholder="Ex: salon_coiffure">
                @error('code')
                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">Identifiant unique utilisé dans la base de données</p>
            </div>

            {{-- Position --}}
            <div>
                <label class="form-label">Position d'affichage</label>
                <input type="number" name="position" value="{{ old('position', $etablissementType->position) }}" min="0" max="999"
            ...
            <div class="flex items-center gap-3">
                <input type="checkbox" id="actif" name="actif" value="1" {{ old('actif', $etablissementType->actif) ? 'checked' : '' }}
