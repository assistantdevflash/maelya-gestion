...
<div class="flex min-h-screen" x-data='{
    ...
    <aside class="fixed inset-y-0 left-0 z-50 w-[260px] flex flex-col lg:translate-x-0 transition-transform duration-300 ease-out"
        ...
        <div class="flex flex-col flex-1 min-h-0 bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-700/60">
            ...
            <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
                ...
                @else
                <a href="{{ route('abonnement.upgrade', ['feature' => 'credits']) }}" class="sidebar-link">
                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
                        <path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4"/>
                    </svg>
                    <span class="flex-1 text-left">Crédits clients</span>
                    <svg class="ml-auto w-3.5 h-3.5 text-amber-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 1a5 5 0 00-5 5v4H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2v-9a2 2 0 00-2-2h-2V6a5 5 0 00-5-5zm-3 9V6a3 3 0 016 0v4H9z"/></svg>
                </a>
                @endif
                ...
                <div x-data="{ open: {{ $boutiqueOpen ? 'true' : 'false' }} }">
                    <button @click="open = !open"
                        ...
                        <span class="flex-1 text-left">Boutique en ligne</span>
                    ...
                    <div x-show="open" x-collapse class="pl-8 mt-1 space-y-1">
                        ...
                        <a href="{{ route('dashboard.boutique.config.index') }}"
                           class="sidebar-link text-sm {{ request()->routeIs('dashboard.boutique.config.index') ? 'active' : '' }}">
                        ...
                        <a href="{{ route('dashboard.boutique.config.marketing') }}"
                           class="sidebar-link text-sm {{ request()->routeIs('dashboard.boutique.config.marketing') ? 'active' : '' }}">
    ...
    {{-- ═══ CONTENU PRINCIPAL ═══ --}}
    <div class="flex-1 flex flex-col min-w-0 lg:pl-[260px] bg-gray-50 dark:bg-slate-950 pb-16 lg:pb-0">
        ...
        <header class="hidden lg:flex sticky top-0 z-30 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border-b border-gray-200/60 dark:border-slate-700/60 px-4 sm:px-6 lg:px-8 h-16 items-center">
            ...
            <div class="flex-1 flex justify-center">
                <button @click="window.openGlobalSearch()"
                    ...
                    <span class="flex-1 text-left">Rechercher client, vente, devis, facture, commande, RDV...</span>
        ...
        <div class="px-6 lg:px-8 pt-4 space-y-2">
            ...
            @php $joursGrace = max(0, 3 - (int) floor(auth()->user()->created_at->floatDiffInDays(now()))); @endphp
            <div class="mb-4 bg-amber-50 dark:bg-amber-900/20 border-2 border-amber-400 dark:border-amber-700 rounded-2xl p-4 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-sm text-amber-700 dark:text-amber-300 mb-3">
                            @if($joursGrace > 0)
                                Il vous reste <strong>{{ $joursGrace }} jour(s)</strong> pour valider votre adresse <strong>{{ auth()->user()->email }}</strong>. Passé ce délai, l'accès sera bloqué.
                            @else
                                Votre délai de 3 jours est dépassé. Veuillez vérifier votre email pour continuer.
                            @endif
                        </p>
                        <a href="{{ route('verification.email') }}"
                           class="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-bold rounded-xl text-white shadow-md active:scale-95 transition-all"
        ...
        <div id="global-search-modal" class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-xl max-h-[70vh] overflow-hidden flex flex-col"
            ...
            <div class="flex items-center gap-3 px-4 py-3 border-b border-gray-100 dark:border-slate-700">
                ...
                <input type="text" id="global-search-input"
                       placeholder="Rechercher client, vente, devis, facture, commande, RDV..."
    ...
    (function() {
        ...
        var icons = {
            ...
            devis: '<svg class="w-3.5 h-3.5 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>',
            facture: '<svg class="w-3.5 h-3.5 text-orange-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 7h6m0 10v-3m-3 3h.01M9 17h.01M9 14h.01M12 14h.01M15 11h.01M12 11h.01M9 11h.01M7 21h10a2 2 0 002-2V5a2 2 0 00-2-2H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>',
            commande: '<svg class="w-3.5 h-3.5 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/></svg>',
