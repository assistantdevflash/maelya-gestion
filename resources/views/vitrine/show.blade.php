    ...
    <section class="mt-6 rounded-2xl overflow-hidden" style="background:linear-gradient(135deg,rgba(147,51,234,0.15),rgba(236,72,153,0.1));border:1px solid rgba(147,51,234,0.2);">
        <div class="px-6 py-10 flex flex-col sm:flex-row sm:items-center gap-5">
            ...
            <div class="flex-1 min-w-0">
                <h1 class="text-2xl font-black text-white leading-tight mb-2">{{ $institut->nom }}</h1>

                {{-- Ligne ville --}}
                @if($institut->ville)
                <p class="flex items-center gap-1.5 text-sm mb-1" style="color:#9ca3af;">
                ...
                {{-- Ligne téléphone --}}
                @if($institut->telephone)
                <a href="tel:{{ $institut->telephone }}" class="inline-flex items-center gap-1.5 text-sm mb-1 transition hover:text-white" style="color:#9ca3af;">
                    <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/>
                    </svg>
                    {{ $institut->telephone }}
                ...
                {{-- Note moyenne --}}
                @if($noteMoyenne)
                <p class="flex items-center gap-1.5 text-sm mb-3" style="color:#eab308;">
                    <svg class="w-4 h-4 fill-current flex-shrink-0" viewBox="0 0 20 20">
                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                    </svg>
                    {{ number_format($noteMoyenne, 1) }}&nbsp;/&nbsp;5
                    <span style="color:#6b7280;">({{ $nbAvis }} avis)</span>
                </p>
                @else
                <div class="mb-3"></div>
                @endif
