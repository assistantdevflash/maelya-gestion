<!DOCTYPE html>
<html lang="fr">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Donnez-nous votre avis</title>
</head>
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#f4f4f7;color:#333;">
@php
    $institut = $rdv?->institut ?? $vente?->institut;
@endphp
<table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f7;">
<tr><td align="center" style="padding:32px 16px;">
<table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);max-width:600px;">

    {{-- HEADER --}}
    <tr>
        <td bgcolor="#9333ea" style="background-color:#9333ea;background:linear-gradient(135deg,#9333ea,#ec4899);padding:36px 32px;text-align:center;">
            <div style="width:56px;height:56px;background:rgba(255,255,255,0.2);border-radius:16px;display:inline-block;text-align:center;line-height:56px;margin-bottom:16px;font-size:28px;">🌸</div>
            <h1 style="color:#ffffff !important;font-size:22px;font-weight:700;margin:0 0 6px;">Merci pour votre visite !</h1>
            @if($rdv)
                <p style="color:rgba(255,255,255,0.85) !important;font-size:14px;margin:0 0 4px;">Bonjour {{ $rdv->client_nom }}, votre avis compte beaucoup pour nous.</p>
                <p style="color:rgba(255,255,255,0.7) !important;font-size:13px;margin:0;">{{ $rdv->institut?->nom ?? config('app.name') }}</p>
            @elseif($vente)
                <p style="color:rgba(255,255,255,0.85) !important;font-size:14px;margin:0 0 4px;">Bonjour {{ $vente->client?->prenom ?? $vente->client?->nom ?? '' }}, votre avis compte beaucoup pour nous.</p>
                <p style="color:rgba(255,255,255,0.7) !important;font-size:13px;margin:0;">{{ $vente->institut?->nom ?? config('app.name') }}</p>
            @endif
        </td>
    </tr>

    {{-- BODY --}}
    <tr>
        <td style="padding:32px;">
            @if($rdv)
                <p style="font-size:15px;color:#374151;margin:0 0 24px;line-height:1.6;">
                    Nous serions ravis de connaître votre avis sur votre dernier rendez-vous du <strong>{{ $rdv->debut_le->translatedFormat('d F Y') }}</strong>.<br>
                    Cela ne prend qu'une minute !
                </p>
            @elseif($vente)
                <p style="font-size:15px;color:#374151;margin:0 0 24px;line-height:1.6;">
                    Nous serions ravis de connaître votre avis sur votre achat du <strong>{{ $vente->created_at->translatedFormat('d F Y') }}</strong>.<br>
                    Cela ne prend qu'une minute !
                </p>
            @endif

            <div style="text-align:center;margin:8px 0 28px;">
                <a href="{{ $lien }}"
                   style="display:inline-block;padding:14px 40px;background:linear-gradient(135deg,#9333ea,#ec4899);color:#fff;text-decoration:none;border-radius:10px;font-weight:700;font-size:15px;">
                    ⭐ Donner mon avis
                </a>
            </div>

            <p style="font-size:13px;color:#6b7280;text-align:center;margin:0;">Ce lien est personnel et à usage unique.</p>
        </td>
    </tr>

    {{-- FOOTER --}}
    <tr>
        <td style="background:#f9fafb;border-top:1px solid #e5e7eb;padding:20px 32px;text-align:center;font-size:12px;color:#9ca3af;">
            @if($institut)
                <p style="margin:0 0 4px;"><strong style="color:#6b7280;">{{ $institut->nom }}</strong></p>
                <p style="margin:0 0 4px;color:#9ca3af;">
                    @if($institut->telephone)📞 {{ $institut->telephone }} &nbsp;·&nbsp; @endif
                    @if($institut->email)✉️ {{ $institut->email }} &nbsp;·&nbsp; @endif
                    @if($institut->ville)📍 {{ $institut->ville }}@endif
                </p>
            @else
                <p style="margin:0 0 4px;"><strong style="color:#6b7280;">{{ config('app.name') }}</strong></p>
                <p style="margin:0 0 4px;color:#9ca3af;">Gestion de salon de beauté</p>
            @endif
            <p style="margin:0;font-size:10px;color:#c4c4c4;">Cet e-mail a été envoyé automatiquement par Maëlya Gestion. Ne pas répondre directement.</p>
        </td>
    </tr>

</table>
</td></tr>
</table>
</body>
</html>
