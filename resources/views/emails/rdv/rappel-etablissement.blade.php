            ...
            <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;margin-bottom:28px;">
                ...
                <tr>
                    <td style="font-size:14px;color:#111827;padding:14px 16px;font-weight:600;">
                        ...
                        <br><span style="font-size:12px;color:#9ca3af;">{{ $rdv->client_telephone }}</span>
                        @endif
                    </td>
                    <td style="font-size:14px;color:#0ea5e9;padding:14px 16px;font-weight:700;">{{ $rdv->debut_le->format('H\hi') }}</td>
                    <td style="font-size:14px;color:#111827;padding:14px 16px;">{{ $rdv->duree_minutes }} min</td>
                </tr>
            </table>

            @if($rdv->prestations->isNotEmpty())
            <p style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#6b7280;margin-bottom:12px;">Prestation(s) prévue(s)</p>
            @foreach($rdv->prestations as $p)
            <p style="margin:4px 0;font-size:14px;color:#374151;">· {{ $p->nom }}@if($p->duree) <span style="color:#9ca3af;">({{ $p->duree }} min)</span>@endif</p>
            @endforeach
            @elseif($rdv->prestation_libre)
            <p style="font-size:14px;color:#374151;"><strong>Prestation :</strong> {{ $rdv->prestation_libre }}</p>
            @endif

            @if($rdv->notes)
            <div style="background:#f0f9ff;border-radius:10px;padding:14px 16px;margin-top:24px;font-size:13px;color:#0369a1;">
                <strong>Note :</strong> {{ $rdv->notes }}
            </div>
            @endif

            <div style="text-align:center;margin:32px 0 8px;">
                <a href="{{ url('/dashboard/rdv/' . $rdv->id) }}"
                   style="display:inline-block;padding:14px 40px;background:linear-gradient(135deg,#0ea5e9,#6366f1);color:#fff;text-decoration:none;border-radius:10px;font-weight:700;font-size:15px;">
                    Voir le rendez-vous →
                </a>
            </div>
        </td>
