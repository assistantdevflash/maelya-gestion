<x-dashboard-layout>
<div class="max-w-3xl mx-auto space-y-8 py-4" x-data="souscrire({{ $plan->prixEffectif() }}, {{ $plan->prixPourPeriode('trimestre') }}, {{ $plan->prixPourPeriode('semestre') }}, {{ $plan->prixPourPeriode('annuel') }}, {{ $plan->prixPourPeriode('triennal') }}, '{{ $periode }}', @js($etablissements->where('optionActive', true)->pluck('id')->values()->all()))">
        ...
        <div class="card">
            ...
            <div class="card-body space-y-4">
                <div class="p-5 bg-gray-50 dark:bg-slate-800 rounded-2xl border-2 border-gray-200 dark:border-slate-600 space-y-3">
                    ...
                    {{-- Option boutique : liste de TOUS les établissements avec cases à cocher --}}
                    <div class="pt-3 border-t-2 border-gray-200 dark:border-slate-600">
                        <div class="flex items-center justify-between gap-2 mb-3">
                            <div class="flex items-center gap-2 flex-wrap">
                                ...
                                <span class="text-sm font-bold text-primary-600 dark:text-primary-300 bg-primary-50 dark:bg-primary-900/50 px-2 py-0.5 rounded">3 900 F/mois / établissement</span>
                            </div>
                            <span class="text-sm font-semibold text-gray-500 dark:text-slate-300">
                                <span x-text="nbBoutiques()"></span> sélectionné<span x-show="nbBoutiques() > 1">s</span>
                            </span>
                        </div>
                        <p class="text-xs text-gray-500 dark:text-slate-400 mb-3">
                            Cochez les établissements pour lesquels vous voulez la boutique en ligne. {{ $estRenouvellement ? 'Ceux déjà actifs restent cochés — décochez pour arrêter, cochez pour ajouter.' : 'Vos clients pourront commander en ligne avec livraison.' }}
                        </p>

                        @forelse($etablissements as $etab)
                        <label class="flex items-center gap-3 cursor-pointer p-3 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-700/50 transition-colors border border-gray-200 dark:border-slate-700 mb-2">
                            <input type="checkbox"
                                   name="boutiques[]"
                                   value="{{ $etab['id'] }}"
                                   x-model="boutiques"
                                   @change="updateBoutiques()"
                                   class="mt-0.5 w-5 h-5 rounded border-gray-300 dark:border-slate-500 text-primary-600 focus:ring-primary-500 dark:bg-slate-700 dark:focus:ring-primary-400">
                            <div class="flex-1 min-w-0">
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold text-gray-900 dark:text-white text-sm truncate">{{ $etab['nom'] }}</span>
                                    @if($etab['optionActive'])
                                    <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300">Actif</span>
                                    @endif
                                </div>
                                @if($etab['ville'])
                                <p class="text-xs text-gray-500 dark:text-slate-400 truncate">{{ $etab['ville'] }}</p>
                                @endif
                            </div>
                        </label>
                        @empty
                        <p class="text-sm text-gray-500 dark:text-slate-400 py-2">
                            Aucun établissement sur ce compte. Créez d'abord un établissement pour activer une boutique.
                        </p>
                        @endforelse

                        <div class="flex items-center justify-between mt-2 px-1">
                            <span class="text-sm text-gray-600 dark:text-slate-300">Sous-total boutique</span>
                            <span class="text-sm font-bold text-primary-600 dark:text-primary-300" x-text="boutiqueTotal() + ' FCFA'"></span>
                        </div>
                        <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">
                            <span x-text="nbBoutiques()"></span> × 3 900 F × <span x-text="periodeMois()"></span> mois
                        </p>
                    </div>
...
<x-slot:scripts>
<script>
function souscrire(prixMensuel, prixTrimestre, prixSemestre, prixAnnuel, prixTriennal, initialPeriode, etablissementsActifs) {
    return {
        ...
        payMethod: 'om',
        copied: false,
        boutiques: etablissementsActifs || [],
        ...
        // Prix total des boutiques pour la période (multi-établissements)
        boutiqueTotal() {
        ...
        updateBoutiques() {
            // Met à jour la réactivité du total (appelé par @change)
            this.boutiques = [...this.boutiques];
        },

        totalPrice() {
            let total = this.planPrice();
            if (this.nbBoutiques() > 0) {
                total += 3900 * this.nbMois() * this.nbBoutiques();
            }
            return new Intl.NumberFormat('fr-FR').format(total);
        },
