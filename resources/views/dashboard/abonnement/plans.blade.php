<x-dashboard-layout>
    <div class="max-w-5xl mx-auto space-y-8 py-4">
        ...
        @php $dansSursis = $abonnementSursis->joursDepuisExpiration() <= 2; @endphp
        <div class="card p-5 {{ $dansSursis ? 'bg-amber-50 border-amber-200' : 'bg-red-50 border-red-200' }} flex items-center gap-4">
            <div class="w-10 h-10 {{ $dansSursis ? 'bg-amber-100 text-amber-600' : 'bg-red-100 text-red-600' }} rounded-full flex items-center justify-center flex-shrink-0">
                @if($dansSursis)
            ...
            <div class="flex-1">
                <p class="font-semibold {{ $dansSursis ? 'text-amber-900' : 'text-red-900' }}">
                    {{ $abonnementSursis->plan->nom ?? 'Abonnement' }} — {{ $dansSursis ? 'Période de sursis' : 'Expiré' }}
                </p>
                <p class="text-sm {{ $dansSursis ? 'text-amber-700' : 'text-red-700' }}">
                    Expiré depuis {{ $abonnementSursis->joursDepuisExpiration() }} jour(s)
                    ({{ $abonnementSursis->expire_le->format('d/m/Y') }})
                    · {{ $dansSursis ? 'Tout fonctionne normalement. Renouvelez avant la fin du sursis.' : 'Accès en lecture seule. Choisissez un plan ci-dessous pour renouveler.' }}
                </p>
            </div>
        </div>
        @endif
                ...
                <div class="relative card overflow-visible flex flex-col {{ $plan->mis_en_avant ? 'border-2 border-primary-400 scale-[1.02]' : '' }} {{ $estPlanActuel ? 'ring-2 ring-emerald-400' : '' }}">
                    ...
                    <div class="p-6 flex-1">
                        ...
                        <ul class="space-y-2.5 text-sm text-gray-600">
                            ...
                            <li class="flex items-center gap-2.5">
                                <svg class="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
                                Edition et suivi des devis et factures
                            </li>
                            <li class="flex items-center gap-2.5">
                                <svg class="w-4 h-4 text-emerald-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg>
