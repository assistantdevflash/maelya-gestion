<x-dashboard-layout>
    ...
    {{-- ═══════ SECTION JOURNAL D'ACTIVITÉ (admin only) ═══════ --}}
    @if(auth()->user()->isAdmin() && isset($logs))
    @php
        $actionLabels = [
    ...
    <div id="journal" class="space-y-4">
        <div class="flex items-center justify-between">
            <div>
                <h2 class="text-xl font-display font-bold text-gray-900 flex items-center gap-2">
                    <svg class="w-5 h-5 text-primary-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"/>
                    </svg>
                    Journal d'activité
                </h2>
                <p class="text-xs text-gray-500 mt-0.5">Toutes les actions sensibles enregistrées sur votre institut</p>
            </div>
            @if($hasFilter)
                <a href="{{ route('dashboard.profil.edit') }}#journal" class="btn-outline text-sm flex items-center gap-1.5">
        ...
        <div class="flex items-center justify-between">
            <p class="text-xs text-gray-400">{{ $logs->total() }} entrée{{ $logs->total() > 1 ? 's' : '' }} au total</p>
            {{ $logs->links() }}
        </div>
    </div>
    @endif
