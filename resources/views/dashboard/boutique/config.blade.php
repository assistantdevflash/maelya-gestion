                ...
                <div class="flex gap-3 mt-4" x-data="{ showModal: false }">
                    ...
                    <div x-show="showModal"
                        ...
                        <div @click.away="showModal = false"
                             x-data="{ methodePaiement: '{{ $paymentMethods->first()?->code ?? 'bank_transfer' }}', copied: false, fileName: '' }"
                            ...
                            <form method="POST" action="{{ route('abonnement.ajouter-boutique') }}" enctype="multipart/form-data" class="p-6 space-y-6">
                                ...
                                {{-- Méthode de paiement (hidden input) --}}
                                <input type="hidden" name="methode_paiement" :value="methodePaiement">
                                ...
                                <div>
                                    ...
                                    <div class="space-y-3">
                                        ...
                                        <label class="flex items-center gap-4 p-4 border-2 rounded-xl cursor-pointer transition-all"
                                               :class="methodePaiement === '{{ $method->code }}' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20' : 'border-gray-200 dark:border-slate-600 hover:border-primary-300 dark:hover:border-primary-700'">
                                            <input type="radio" name="methode_radio" value="{{ $method->code }}" class="w-5 h-5 text-primary-600" @click="methodePaiement = '{{ $method->code }}'" :checked="methodePaiement === '{{ $method->code }}'">
                                            <div class="flex-1">
                                                <div class="flex items-center gap-2">
                                                    @if($method->code === 'geniuspay')
                                                    <svg class="w-5 h-5 text-purple-600 dark:text-purple-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
                                                    @else
                                                    <svg class="w-5 h-5 text-gray-600 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z"/></svg>
                                                    @endif
                                                    <span class="font-semibold text-gray-900 dark:text-white">{{ $method->name }}</span>
                                                </div>
                                                @if($method->description)
                                                <p class="text-sm text-gray-600 dark:text-slate-400 mt-1">{{ $method->description }}</p>
                                                @endif
                                            </div>
                                        </label>
                                        @endforeach
                                ...
                                <div>
                                    <button type="submit" x-show="methodePaiement === 'bank_transfer'" class="btn-primary w-full py-4 text-base">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
                                        Envoyer ma demande
                                    </button>
                                    <button type="submit" x-show="methodePaiement === 'geniuspay'" class="btn-primary w-full py-4 text-base bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                                        Procéder au paiement GeniusPay
                                    </button>
                                </div>
                            </form>
                        </div>
