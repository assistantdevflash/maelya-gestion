                ...
                <div class="mt-3 border-t border-gray-100 dark:border-gray-700 pt-3 space-y-2">
                    ...
                    <div class="flex items-center justify-between gap-3 px-1">
                        ...
                        <form method="POST" action="{{ route('dashboard.mes-instituts.vitrine', $institut) }}">
                            @csrf @method('PATCH')
                            <button type="submit"
                                    class="relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-1 {{ $institut->vitrine_active ? 'bg-emerald-500' : 'bg-gray-400 dark:bg-gray-500' }}"
                                    title="{{ $institut->vitrine_active ? 'Désactiver' : 'Activer' }} la page publique">
                                <span class="pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out {{ $institut->vitrine_active ? 'translate-x-5' : 'translate-x-0' }}"></span>
                            </button>
                        </form>
                    </div>
