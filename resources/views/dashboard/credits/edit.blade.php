<x-dashboard-layout>
<div class="max-w-4xl mx-auto space-y-6" x-data="creditForm(@js($allClients->toArray()), @js($catalogue->toArray()), @js($editData))">
    <div class="flex items-center gap-4">
        <a href="{{ route('dashboard.credits.show', $credit) }}" class="p-2 text-gray-400 hover:text-gray-600 rounded-xl hover:bg-gray-100 dark:hover:bg-slate-800 transition">←</a>
        <div><h1 class="text-2xl font-bold text-gray-900 dark:text-white">Modifier le crédit</h1></div>
    </div>

    <form method="POST" action="{{ route('dashboard.credits.update', $credit) }}">
        @csrf
        @method('PUT')
                ...
                <div class="card" style="overflow: visible">
                    ...
                    <div class="card-body space-y-3">
                        <template x-for="(ligne, i) in lignes" :key="i">
                            <div class="flex flex-wrap items-center gap-2 p-3 bg-gray-50 dark:bg-slate-800 rounded-xl" x-transition.opacity.duration.300>
